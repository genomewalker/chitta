...
# /projects/caeg/scratch (eval-replica.sh's default) and compiler TMPDIR is chosen privately on the compute node.
#
...
set -euo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/build-env.sh"
...
exec srun --quiet -N1 -n1 -c "$cpus" --mem="$mem" -t "$time" \
    ...
    --chdir="$PWD" --export=ALL,OMP_NUM_THREADS="${OMP_NUM_THREADS:-$cpus}",OPENBLAS_NUM_THREADS="${OPENBLAS_NUM_THREADS:-$cpus}",RAYON_NUM_THREADS="${RAYON_NUM_THREADS:-$cpus}" \
    "$@"
