...
def band(data: dict, metric: str) -> float:
    if not data.get("acceptance_ready"):
        raise ValueError("not calibrated on a frozen replica with a real agent; band unavailable")
...
def calibrate(args) -> dict:
    ...
    return {
        ...
        "accept_rule": "improvement strictly beyond 2 * sd; positive SR/nDCG, negative tokens",
        "metrics": metrics,
        "smriti_records": rows,
    }
...
def main():
    ...
    try:
        if args.command == "band":
            ...
            if result["errors"] or (args.agent == "claude-code" and not result["acceptance_ready"]):
