...
export CHITTA_BIN="$T/bin/chitta" CHITTA_QUEUE="$T/queue.jsonl" CHITTA_REALM=project:test CHITTA_RECALL_LANES_RPC=0
...
topic=$(grep -cE '^(smart_recall|recall --query Do all --strategy (hybrid|keyword))' "$T/zero.log" || true)
...
topic=$(grep -cE '^(smart_recall|recall --query .* --strategy (hybrid|keyword))' "$T/many.log" || true)
