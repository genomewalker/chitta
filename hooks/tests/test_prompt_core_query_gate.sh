...
"${CXX:-g++}" -std=c++17 -O2 -pthread -I"$here/../../chitta/include" \
    "$here/event-response.cpp" -lcrypto -o "$T/bin/chitta" || exit 1
...
run_prompt() {
    export STUB_PREPARED_STATE="$T/$2.json"
...
topic=$(jq '[.retrieval.lanes[] | select(. == "sem" or . == "kw" or . == "hyb")] | length' "$T/zero.json")
...
topic=$(jq '[.retrieval.lanes[] | select(. == "sem" or . == "kw" or . == "hyb")] | length' "$T/many.json")
