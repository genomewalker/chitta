...
def main():
    ...
    cases = [
        ...
        (
            "shape-gates",
            [dict(r, cmd_head=None) for r in rows] + [dict(r, cmd_head="\t ") for r in rows],
            cmd,
            {},
            False,
        ),
        ...
        (
            "interleaved",
            [r for row in rows for r in (row, dict(row, cmd_head="git status", exit_code=0))],
            cmd,
            {},
            True,
        ),
        (
            "normalization",
            [dict(r, cmd_head="  CURL\tHTTPS://broken.test/retry/123\n") for r in rows],
            cmd,
            {},
            True,
        ),
        (
            "sixty-characters",
            [dict(r, cmd_head="x" * 60 + str(i)) for i, r in enumerate(rows)],
            "x" * 60 + "different",
            {},
            True,
        ),
        (
            "near-similar",
            [dict(r, cmd_head="abcdefghij" + x) for r, x in zip(rows, ("aa", "ab", "ac"))],
            "abcdefghijad",
            {},
            True,
        ),
        ...
        (
            "sequence-not-edit-distance",
            [dict(r, cmd_head="tide") for r in rows],
            "diet",
            {"similarity": 0.4},
            False,
        ),
        (
            "sequence-asymmetric",
            [dict(r, cmd_head="diet") for r in rows],
            "tide",
            {"similarity": 0.4},
            True,
        ),
        (
            "unicode",
            [dict(r, cmd_head="ÉCHO\u001cΩΣ\t１２３", stderr_head="é\t💥\n" * 80) for r in rows],
            "écho ως ٩",
            {},
            True,
        ),
    ...
    cases.append(
        (
            "close-multiple",
            tied + [dict(rows[-1], ts=now, cmd_head="abcxy", exit_code=0)],
            "abcxy",
            {"similarity": 0.6},
            False,
        )
    ...
    with tempfile.TemporaryDirectory(prefix="saddle-parity-") as tmp:
        ...
        for name, events, command, options, expected in cases:
            ...
            args = SimpleNamespace(
                session=sid,
                cmd=command,
                min_fails=options.get("min_fails", 3),
                minutes=options.get("minutes", 7),
                similarity=options.get("similarity", 0.8),
            ...
            assert result.stdout == out.getvalue().encode(), (
                name,
                result.stdout,
                out.getvalue(),
                result.stderr,
