...
def rewrite(command,**env):
 p=subprocess.run(['bash',str(root/'hooks/pre-tool-hook.sh'),'Bash'],input=json.dumps({'session_id':'cap-test','tool_input':{'command':command,'timeout':123}}),text=True,capture_output=True,env=dict(os.environ,**env),check=True)
