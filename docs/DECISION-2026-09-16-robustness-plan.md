...
### Phase 9 — Code navigation instead of file reading (owner request 2026-09-16)
- Finding: chitta indexes 121,236 symbols store-wide but only 440 for this repo; its code tools answer per symbol; the pre-read hook is advisory; nothing gives the agent a repo map. Graphify's pattern (tree-sitter graph with communities and god nodes, `query` returning a scoped subgraph, a pre-read hook pointing at it, rebuild on commit and checkout) is what stops agents reading files.
- Steps: (1) complete and current index for the active repo: full `learn_codebase` when indexed files fall below `git ls-files`, rebuild on commit and checkout; (2) `code_query` RPC: question → scoped subgraph (symbols, files, `calls`/`imports`/`inherits` edges with EXTRACTED/INFERRED confidence) plus `codebase_overview` with communities and god nodes, rendered as a short block at session start; (3) pre-read injection: before a Read of an indexed file, the file's symbols with callers and callees, so `read_symbol` is the cheap choice; a benchmark of 20 navigation questions about this repo with file:symbol ground truth.
...
- **GLiNER 2.5 spike (owner question 2026-09-17)**: fastino/gliner2.5-base-v1 (194M, Apache 2.0, local, entities/classification/records/relations with confidences) measured against four heuristic or LLM paths: triplet extraction from distillation inputs, capsule next-action extraction versus the awk rule, Bash outcome classification versus the ledger rule, answer-span grading versus the current-truth matcher; CPU and one A100 (compregular has gpu:a100:2 on one node). Task file codex-specs/p11-gliner.md; runs at medium effort once the Phase 10 and Phase 2 streams finish, to keep concurrency at two.
